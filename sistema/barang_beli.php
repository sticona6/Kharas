...
if($stok>0)
{
	if(isset($_GET['Kode'])) {
		...
		$cekQry = mysqli_query($koneksidb, $cekSql) or die ("Compruebe los datos ingresados".mysqli_error());
		if(mysqli_num_rows($cekQry) >=1) {
			...
			$myQry = mysqli_query($koneksidb, $mySql) or die ("Error en los datos Ingresados".mysqli_error());
