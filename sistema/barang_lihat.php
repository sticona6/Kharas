...
if (isset($_GET['Kode'])) {
	...
	$lihatQry= mysqli_query($koneksidb, $lihatSql) or die("Datos No Encontrados".mysqli_error());
	$no=0;
...
$barangSql =mysqli_query($koneksidb, "SELECT * FROM barang where kd_kategori='$codKategoria' ORDER BY kd_barang");
//$barangQry = mysql_query($barangSql, $koneksidb) or die ("Gagal Query".mysql_error()); 
$nomor = 0;
while ($barangData = mysqli_fetch_array($barangSql)) 
{
